...
namespace ChartAndGraph
{
    public partial class GraphChart : GraphChartBase, ICanvas
    {
        ...
        class CategoryObject
        {
            ...
            public Dictionary<int, (DoubleVector3, string)> mCahced = new Dictionary<int, (DoubleVector3, string)>();
        }
        ...
        public override void GenerateRealtime()
        {
            ...
            foreach (GraphData.CategoryData data in ((IInternalGraphData)Data).Categories)
            {
                ...
                obj.mCahced.Remove(mTmpData.Count - 2); // never store the last point cache , it might be intepolating by the realtime feature
                ...
                if (mItemLabels != null && mItemLabels.isActiveAndEnabled && obj.mItemLabels != null)
                {
                    ...
                    for (int i = 0; i < mTransformed.Count; i++)
                    {
                        ...
                        if (obj.mCahced.TryGetValue(pointIndex, out toSet) == false || toSet.Item1!= mTmpData[i + refrenceIndex])
                        {
                        ...
                        BillboardText billboard = m.AddText(this, mItemLabels.TextPrefab, parentT, mItemLabels.FontSize, mItemLabels.FontSharpness, toSet.Item2, labelPos.x, labelPos.y, labelPos.z, 0f, null);
