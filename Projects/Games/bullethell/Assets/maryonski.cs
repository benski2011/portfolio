...
/// </summary>
public class maryonski : EnemyBaseScript
{
    ...
    public GameObject initRayPrefab;
    ...
    private State currentState = State.Ray;
    ...
    void FixedUpdate()
    {
        ...
        if (initray && !rayFired)
        {
            initray.transform.position = barrel.transform.position;
            Vector3 direction = playermidpoint.transform.position - initray.transform.position;
            direction.y = 0;
            direction = direction.normalized;

            initray.transform.rotation = Quaternion.FromToRotation(Vector3.forward, direction);
        ...
        if (initray && currentState != State.Ray )
        {
            Destroy(initray);
        }
    ...
    IEnumerator FireRay()
    {
        Vector3 direction = playermidpoint.transform.position - barrel.transform.position;
    ...
    private void ShootRay()
    {
        if (initray == null)
        {
            ...
            initray = Instantiate(initRayPrefab);
